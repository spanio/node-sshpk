...
let ECDSA_ALGO = /^ecdsa-sha2-([^@-]+)-cert-v01@openssh.com$/;
...
async function listKeys() {
	...
	return await client.send(command)
		...
		.catch(err => {
			console.trace(err); // an error occurred
		});
}
...
async function sign(cert, key) {
	...
	return client.send(command)
		.then(data => {
			...
			let sig = Signature.parse(signature, key.type, 'asn1');
...
async function read_bkp(buf, options) {
